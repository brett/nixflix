...
in
{
  ...
  config = mkMerge [
    # Always create user/group/state dirs when enabled — host needs them for virtiofs ownership
    (mkIf (nixflix.enable && cfg.enable) {
      ...
      services.nginx = mkIf nixflix.nginx.enable {
        virtualHosts.localhost.locations =
          let
            proxyHost = if cfg.microvm.enable then cfg.microvm.address else "127.0.0.1";
          in
          {
            "/${cfg.network.baseUrl}" = {
