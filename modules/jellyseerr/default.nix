...
in
{
  ...
  config = mkMerge [
    # Always create user/group/state dir when enabled — host needs them for virtiofs ownership
    (mkIf (nixflix.enable && cfg.enable) {
      ...
      systemd.tmpfiles.rules =
        let
          # In microVM mode the guest runs the service as user:media for virtiofs
          # ownership to work correctly; the host dir must match.
          group = if cfg.microvm.enable then globals.libraryOwner.group else cfg.group;
        in
        [
    ...
    # Only run the service on the host when not in microVM mode
    (mkIf (nixflix.enable && cfg.enable && !cfg.microvm.enable) {
      assertions =
        let
