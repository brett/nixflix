# Shared Jellyfin API verification logic.
# Returns a Python script fragment for use in NixOS test scripts.
#
# Prerequisites — must be defined in the calling test script before interpolating:
#   machine      - the NixOS test machine object
#   auth_header  - the Authorization header string,
#                  e.g. '"Authorization: MediaBrowser Client=..., Token=\"...\""'
#
# Parameters:
#   host  - Jellyfin hostname/IP (Nix string, e.g. "127.0.0.1" or "10.100.0.30")
#   port  - Jellyfin HTTP port (Nix int, default 8096)
{
  host,
  port ? 8096,
}:
''
  base_url = "http://${host}:${toString port}"
  ...
  assert len(kiri_users) == 1, f"Expected 1 kiri user, found {len(kiri_users)}"
  user = kiri_users[0]
