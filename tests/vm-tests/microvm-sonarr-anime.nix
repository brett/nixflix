# MicroVM test: sonarr-anime runs in an isolated microVM and the API is
# reachable from the host at the VM's static IP.
# Verifies root folder, delay profile, and SABnzbd download client — the
# same configuration surfaces tested by sonarr-anime-basic.nix.
{
...
if microvmModules == null then
  pkgs.runCommand "microvm-sonarr-anime-skip" { } ''
    echo "microvm-sonarr-anime: skipped (pass microvmModules to run)" > $out
  ''
else
  let
  ...
  base.pkgsUnfree.testers.runNixOSTest {
    name = "microvm-sonarr-anime-test";
        ...
        nixflix = {
          ...
          sonarr-anime = {
            ...
            config = {
              hostConfig = {
                port = 8990;
                # password triggers creation of sonarr-anime-config.service inside the guest.
                password = {
                  _secret = pkgs.writeText "sonarr-anime-password" "testpassword123";
                };
              };
              apiKey = {
                _secret = pkgs.writeText "sonarr-anime-apikey" "abcdef0123456789abcdef0123456789";
              };
              delayProfiles = [
                {
                  enableUsenet = true;
    ...
    testScript = ''
      ...
      machine.succeed("mkdir -p /data/.state/postgres /data/.state/sonarr-anime /data/.state/sabnzbd /data/media /data/downloads /media/anime")

      machine.wait_for_unit("microvm@postgres.service", timeout=600)
      machine.wait_for_unit("microvm@sonarr-anime.service", timeout=600)
      machine.wait_for_unit("sabnzbd.service", timeout=600)

      # Config service inside the VM restarts sonarr-anime after first startup; poll until stable.
      result = machine.wait_until_succeeds(
          "curl -sf -H 'X-Api-Key: abcdef0123456789abcdef0123456789' "
      ...
      machine.wait_until_succeeds(
          "curl -sf -H 'X-Api-Key: abcdef0123456789abcdef0123456789' "
          "http://10.100.0.11:8990/api/v3/rootfolder | grep -q '/media/anime'",
          timeout=120
      )
      print("Root folder /media/anime verified")

      import json
      machine.wait_until_succeeds(
          "curl -sf -H 'X-Api-Key: abcdef0123456789abcdef0123456789' "
          "http://10.100.0.11:8990/api/v3/delayprofile | grep -q 'torrent'",
          timeout=120
      )
      profiles_list = json.loads(machine.succeed(
          "curl -sf -H 'X-Api-Key: abcdef0123456789abcdef0123456789' "
          "http://10.100.0.11:8990/api/v3/delayprofile"
      ))
      ...
      machine.wait_for_unit("sonarr-anime-downloadclients.service", timeout=180)
      clients_list = json.loads(machine.succeed(
          "curl -sf -H 'X-Api-Key: abcdef0123456789abcdef0123456789' "
          "http://10.100.0.11:8990/api/v3/downloadclient"
      ))
      ...
      assert category_field['value'] == 'sonarr-anime', f"Expected tvCategory 'sonarr-anime', got: {category_field['value']}"
      print("SABnzbd download client with tvCategory='sonarr-anime' verified")
      ...
      machine.succeed(
          ...
          "http://10.100.0.11:8990/api/v3/downloadclient/test"
      )
      print("Sonarr-Anime → SABnzbd cross-VM connectivity verified")
      ...
      profiles = json.loads(machine.succeed(
          ...
          "http://10.100.0.11:8990/api/v3/qualityprofile"
      ))
      assert len(profiles) > 0, f"Expected quality profiles from postgres DB, got: {profiles}"
      print(f"postgres DB connectivity verified from sonarr-anime VM: {len(profiles)} quality profile(s) present")

      # The postgres VM firewall blocks the host bridge IP from port 5432.
      machine.fail("bash -c 'echo >/dev/tcp/10.100.0.2/5432'")

      print("microvm-sonarr-anime: API, root folder, delay profile, download client, cross-VM connectivity, and postgres all verified")
    '';
  }
